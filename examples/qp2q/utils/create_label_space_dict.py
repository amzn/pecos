...
def create_label_dict(fdir, f_compressed, out_file, add_suffix):
    ...
    if add_suffix:
        ...
        train_next_queries = {q_suffix:1 for q in train_next_queries for q_suffix in get_suffixes(q)}
...
def main():
    ...
    parser.add_argument('--out_file', type=str, required=True, help="name of output label dictionary file")
    parser.add_argument('--f_compressed', type=int, choices=[1, 0], default=0, help="are files in train data directory compressed. 1 for yes and 0 for no")
    parser.add_argument('--add_suffix', type=int, choices=[1, 0], default=0, help="Add suffixes of all labels in the label dictionary. 1 for yes and 0 for no")
    ...
    create_label_dict(fdir=_fdir, out_file=_out_file, f_compressed=_f_compressed, add_suffix=_add_suffix)
