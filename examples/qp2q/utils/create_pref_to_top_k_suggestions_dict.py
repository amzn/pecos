...
def get_top_k_w_label_trie(prefix, label_trie, k):
    if label_trie.has_subtrie(prefix):  # if prefix exists in trie
        sorted_labels = [(label, freq) for label, freq in
                         sorted(label_trie.iteritems(prefix=prefix), key=lambda x: x[1], reverse=True)]
...
def create_pref_to_topk_dict(fdir, f_compressed, fname, k):
    ...
    all_prefs_to_topk = {pref:get_top_k_w_label_trie(prefix=pref, label_trie=label_trie, k=k) for pref in tqdm(all_prefs)}
...
def main():
    parser = argparse.ArgumentParser(description='Create a dict where key = prefix of a label and value = top-k suggestions matching the given prefix')
    ...
    parser.add_argument('--k', type=int, required=True, help="max number of suggestions to store for each prefix")
