...
namespace ann {
    ...
    struct PairwiseANN {
        ...
        void train(const mat_t &X_trn, const pecos::csr_t &Y_trn) {
            ...
            if (X_trn.rows != Y_trn.rows) {
                throw std::runtime_error("X_trn.rows != Y_trn.rows");
            }
            // scalar variables
            this->num_input_keys = Y_trn.rows;
            this->num_label_keys = Y_trn.cols;
            this->feat_dim = X_trn.cols;
            // matrices
            this->Y_csc = Y_trn.to_csc();
            this->X_trn = X_trn;
        }
