...
def get_all_data(fname):
    ...
    with open(fname, 'r') as f:
        ...
        for ctr, row in enumerate(reader):
            ...
            query_tokens = [re.sub(r'\W+', '', token) for token in query_tokens] # Remove non-alpha numberic characters
...
def create_sessions(all_data, sess_len):
    ...
    """
    LOGGER.info("Splitting data into session using min query time diff = {} seconds".format(sess_len))
    all_session_data = []
            ...
            for curr_query, curr_query_time in all_data[user_id][1:]:
                ...
                if diff.total_seconds() > sess_len: # Start new session
                    if len(curr_session) > 0:
                        all_session_data    += [(curr_session, min(curr_session_times), max(curr_session_times))]
    ...
    LOGGER.info("Created data w/ {} sessions using min query time diff = {} seconds".format(len(all_session_data), sess_len))
...
def deduplicate_session_data(all_session_data):
    ...
    LOGGER.info("Removing duplicate consecutive queries from {} sessions".format(len(all_session_data)))
    ...
    LOGGER.info("Finished removing duplicate consecutive queries and we have {} sessions now ".format(len(dedup_sess_data)))
...
def split_sessions_into_train_test_val(all_sessions):
    ...
    """
    date_format = '%Y-%m-%d %H:%M:%S'
    train_time_range    = datetime.strptime('2006-03-01 00:00:00', date_format), datetime.strptime('2006-05-15 23:59:59', date_format)
    val_time_range      = datetime.strptime('2006-05-16 00:00:00', date_format), datetime.strptime('2006-05-23 23:59:59', date_format)
    test_time_range     = datetime.strptime('2006-05-24 00:00:00', date_format), datetime.strptime('2006-05-31 23:59:59', date_format)
    train_sessions, test_sessions, val_sessions  = [], [], []
    for curr_session, min_sess_time, max_sess_time in all_sessions:

        assert min_sess_time <= max_sess_time, "Min session = {} time should be less than max session time = {}.".format(min_sess_time, max_sess_time)
        ...
        if train_time_range[0] <= min_sess_time <= train_time_range[1]:
            ...
            raise Exception("This session range is not handled = ({}, {})".format(min_sess_time, max_sess_time))

    return train_sessions, test_sessions, val_sessions


def create_query_pairs(all_session_data):
    query_pairs = [(prev_query, next_query) for s_queries in all_session_data
...
def convert_to_train_data_format(query_pairs, out_fname):
    ...
    with open(out_fname, "w") as writer:
        for prev_query, next_query in query_pairs:
            ...
            new_datapoint = {"prev_query": prev_query,
...
def convert_to_gt_data_format(query_pairs, out_fname):
    ...
    with open(out_fname, "w") as writer:
        for prev_query, next_query in query_pairs:
            pref_len = int(np.random.choice(range(1, len(next_query)+1), size=1)[0]) # Uniformly sample a prefix
            prefix = next_query[:pref_len]
            output = {"prev_query": prev_query,
                      "prefix": prefix,
                      "next_query": next_query
                      }
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, required=True, help="Directory where all raw AOL log files are stored")
    parser.add_argument('--out_dir', type=str, required=True,  help="Dir to store out file")
    parser.add_argument('--sess_len', type=int, default=1800,  help="Min time gap (in seconds) b/w two consecutive queries to put them in different sessions ")
    ...
    os.system("echo {} >> {}/command_to_run.txt".format(" ".join([str(x) for x in sys.argv]), out_dir))
    ...
    for curr_file in data_files:
        ...
        dedup_sessions = deduplicate_session_data(all_session_data=create_sessions(all_data=all_data, sess_len=sess_len))
        ...
        assert (len(train_sessions) + len(val_sessions) + len(test_sessions) == len(all_sessions)), "{} + {} + {} != {}".format(len(train_sessions), len(val_sessions), len(test_sessions), len(all_sessions))

    LOGGER.info("Total number of sessions = {}".format(len(all_sessions)))
    LOGGER.info("Total number of query pairs = {}".format(len(create_query_pairs([sess for sess, _,_ in all_sessions]))))
    LOGGER.info("Total number of unique query pairs = {}".format(len(set(create_query_pairs([sess for sess, _,_ in all_sessions])))))
    LOGGER.info("Total number of queries = {}".format(len([q for sess,_,_ in all_sessions for q in sess])))
    LOGGER.info("Total number of unique queries = {}".format(len(set([q for sess,_,_ in all_sessions for q in sess]))))

    LOGGER.info("Total number of train sessions = {}".format(len(train_sessions)))
    LOGGER.info("Total number of train query pairs = {}".format(len(create_query_pairs(train_sessions))))
    LOGGER.info("Total number of train unique query pairs = {}".format(len(set(create_query_pairs(train_sessions)))))
    LOGGER.info("Total number of train queries = {}".format(len([q for sess in train_sessions for q in sess])))
    LOGGER.info("Total number of unique train queries = {}".format(len(set([q for sess in train_sessions for q in sess]))))

    LOGGER.info("Total number of test sessions = {}".format(len(test_sessions)))
    LOGGER.info("Total number of test query pairs = {}".format(len(create_query_pairs(test_sessions))))
    LOGGER.info("Total number of test unique query pairs = {}".format(len(set(create_query_pairs(test_sessions)))))
    LOGGER.info("Total number of test queries = {}".format(len([q for sess in test_sessions for q in sess])))
    LOGGER.info("Total number of unique test queries = {}".format(len(set([q for sess in test_sessions for q in sess]))))

    LOGGER.info("Total number of val sessions = {}".format(len(val_sessions)))
    LOGGER.info("Total number of val query pairs = {}".format(len(create_query_pairs(val_sessions))))
    LOGGER.info("Total number of val unique query pairs = {}".format(len(set(create_query_pairs(val_sessions)))))
    LOGGER.info("Total number of val queries = {}".format(len([q for sess in val_sessions for q in sess])))
    LOGGER.info("Total number of unique val queries = {}".format(len(set([q for sess in val_sessions for q in sess]))))

    convert_to_train_data_format(query_pairs=create_query_pairs(train_sessions), out_fname="{}/train/train.json".format(out_dir))
    convert_to_train_data_format(query_pairs=create_query_pairs(val_sessions), out_fname="{}/val/val.json".format(out_dir))
    convert_to_gt_data_format(query_pairs=create_query_pairs(test_sessions), out_fname="{}/test/test.json".format(out_dir))
