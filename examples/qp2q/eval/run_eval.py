...
def eval_helper(model_path, gt_file, gen_pred_data, eval_latency, out_dir):
    ...
    if gen_pred_data:
        LOGGER.info("\n Generating data for for model : {m} in res dir :{o} using ground-truth at {g} \n".format(m=model_path, o=out_dir, g=gt_file))
        generate_predictions(config_dict=config_dict, pred_data_path=pred_data_path, gt_data_file=gt_file)
    ...
    if eval_latency:
        ...
        eval_pred_latency(config_dict=config_dict, gt_data_file=gt_file, pred_data_path=pred_data_path, num_samples=100000, warmup_samples=1000)
...
def main():
    ...
    parser.add_argument('--gt',
                        required=True,
                        type=str,
                        help="Path to file with ground-truth data")
    parser.add_argument('--out_dir',
                        required=True,
                        type=str,
                        help="Directory to save eval result")
    parser.add_argument('--gen_pred_data',
                        default=1,
                        type=int,
                        help="Whether to generate pred data or not")
    parser.add_argument('--eval_latency', default=0,
                        type=int,
    ...
    if os.path.isfile(gt_file):
        eval_helper(model_path=model_path, gt_file=gt_file, gen_pred_data=gen_pred_data, eval_latency=eval_latency, out_dir=out_dir)
    else:
