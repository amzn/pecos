...
// Memory-mappable vector of std::pair<uint32_t, uint64_t> for Ankerl
// This vector takes/gets std::string_view as the key and the key should be of fixed length
class AnkerlFixedLenStr2IntMmapableVector {
    template <bool IsConst>
    class iter_t;

    public:
        using key_type = std::string_view;
        using value_type = std::pair<uint32_t, uint64_t>;
        using size_type = std::size_t;
        using difference_type = std::ptrdiff_t;
        using allocator_type = std::allocator<value_type>;
        using reference = value_type&;
        ...
        AnkerlFixedLenStr2IntMmapableVector() = default;
        AnkerlFixedLenStr2IntMmapableVector(allocator_type alloc)
            : store_(alloc) {}
        ...
        /* Emplace string-like key and int value as std::pair<uint32_t, uint64_t>*/
        template <typename K, typename... Args>
        auto emplace_back(std::piecewise_construct_t, std::tuple<K> key, std::tuple<Args...> args) {
            // Extract key
            key_type k = std::get<0>(key);

            // Emplace back std::pair<uint32_t, uint64_t>
            auto eb_val = store_.emplace_back(
                std::piecewise_construct,
        ...
        key_type get_key(value_type const& vt) const {
            auto index = vt.first;
            return key_type(str_data_ + (index * fixed_str_len_), fixed_str_len_);
        }
        ...
        void save_to_mmap_store(pecos::mmap_util::MmapStore& mmap_s) const {
            mmap_s.fput_one<size_type>(size_);
            mmap_s.fput_one<size_type>(str_size_);
            mmap_s.fput_one<size_type>(fixed_str_len_);
            mmap_s.fput_multiple<value_type>(data_, size_);
            mmap_s.fput_multiple<char>(str_data_, str_size_);
        }
        ...
        void load_from_mmap_store(pecos::mmap_util::MmapStore& mmap_s) {
            ...
            size_ = mmap_s.fget_one<size_type>();
            str_size_ = mmap_s.fget_one<size_type>();
            fixed_str_len_ = mmap_s.fget_one<size_type>();
            data_ = mmap_s.fget_multiple<value_type>(size_);
            str_data_ = mmap_s.fget_multiple<char>(str_size_);
        }
    ...
    private:
        ...
        class iter_t {
            using ptr_t = typename std::conditional_t<IsConst,
                AnkerlFixedLenStr2IntMmapableVector::const_pointer, AnkerlFixedLenStr2IntMmapableVector::pointer>;
            ptr_t iter_data_{};
            ...
            public:
                using iterator_category = std::forward_iterator_tag;
                using difference_type = AnkerlFixedLenStr2IntMmapableVector::difference_type;
                using value_type = AnkerlFixedLenStr2IntMmapableVector::value_type;
                using reference = typename std::conditional_t<IsConst,
                    value_type const&, value_type&>;
                using pointer = typename std::conditional_t<IsConst,
                    AnkerlFixedLenStr2IntMmapableVector::const_pointer, AnkerlFixedLenStr2IntMmapableVector::pointer>;
