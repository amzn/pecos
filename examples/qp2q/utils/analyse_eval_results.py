...
def calc_mrr(gt_data_iterator, pred_data_iterator):
    all_mrr = []
    all_bleu = []
    for j, (gt_label, preds) in tqdm(enumerate(zip(gt_data_iterator, pred_data_iterator)), total=len(gt_data_iterator)):
        ...
        normalizer = 0.
        for i, curr_pred in enumerate(preds):
            wgtd_bleu_score += bleu_score.sentence_bleu([gt_label.split()], curr_pred.split(),
                                                        smoothing_function=SmoothingFunction().method1) / (i + 1)
            normalizer += 1.0 / (i + 1)
        wgtd_bleu_score = wgtd_bleu_score / normalizer if normalizer > 0 else wgtd_bleu_score
        all_bleu.append(wgtd_bleu_score)

    return {"mrr": np.mean(all_mrr) if len(all_mrr) > 0 else 0.,
            "mrr_err": np.std(all_mrr) if len(all_mrr) > 0 else 0.,
            "bleu": np.mean(all_bleu) if len(all_bleu) > 0 else 0.,
            "bleu_err": np.std(all_bleu) if len(all_bleu) > 0 else 0.,
...
def get_bucketed_scores(feat_to_data, feat_buckets):
    ...
    for min_feat_val, max_feat_val in zip(feat_buckets[:-1], feat_buckets[1:]):
        ...
        feat_bkt_to_scores[min_feat_val, max_feat_val] = calc_mrr(gt_data_iterator=gt_data,
                                                                       pred_data_iterator=pred_data)
...
def analyse(gt_filename, pred_filename, label_trie, only_seen_labels):
    ...
    freq_to_data, pref_len_to_data = get_data_by_freq_and_pref_len(gt_filename=gt_filename, pred_filename=pred_filename,
                                                                   label_trie=label_trie, only_seen_labels=only_seen_labels)

    freq_to_scores = {freq: calc_mrr(gt_data_iterator=gt_data, pred_data_iterator=pred_data)
                      for freq, (gt_data, pred_data) in freq_to_data.items()}

    pref_len_to_scores = {pref_len: calc_mrr(gt_data_iterator=gt_data, pred_data_iterator=pred_data)
    ...
    write_results(pred_fname=pred_filename, feat_name="freq_bkt", feat_to_scores=bktd_freq_to_scores, res_file=res_file,
                  gt_fname=gt_filename)

    res_file = f"{out_dir}/{test_fname}_eval_res_by_freq_{only_seen_labels}.csv"
    write_results(pred_fname=pred_filename, feat_name="freq", feat_to_scores=freq_to_scores, res_file=res_file, gt_fname=gt_filename)

    res_file = f"{out_dir}/{test_fname}_eval_res_by_freq_bkt_{only_seen_labels}.csv"
    bktd_freq_to_scores = get_bucketed_scores(feat_to_data=freq_to_data, feat_buckets=[0, 1, 32, 128, 512, 2048, 8192, 32000, 80000])
    write_results(pred_fname=pred_filename, feat_name="freq_bkt", feat_to_scores=bktd_freq_to_scores, res_file=res_file, gt_fname=gt_filename)
    ...
    write_results(pred_fname=pred_filename, feat_name="pref_len", feat_to_scores=pref_len_to_scores, res_file=res_file, gt_fname=gt_filename)
    ...
    res_file = f"{out_dir}/{test_fname}_eval_res_by_pref_len_bkt_{max_pref_bkt}_{only_seen_labels}.csv"
    bktd_pref_len_to_scores = get_bucketed_scores(feat_to_data=pref_len_to_data,  feat_buckets=pref_feat_bkts)
    write_results(pred_fname=pred_filename, feat_name="pref_len_bkt", feat_to_scores=bktd_pref_len_to_scores, res_file=res_file, gt_fname=gt_filename)


def main():
    parser = argparse.ArgumentParser(description='Breakdown performance of model given prediction file')
    parser.add_argument('--gt_file', type=str, required=True, help="file containing ground-truth data")
    parser.add_argument('--pred_file', type=str, required=True, help="file containing prediction data")
    parser.add_argument('--label_file', type=str, default=None, help="file containing label freq trie")
...
def final_paper_analysis():
    ...
    parser.add_argument('--base_dir', type=str, required=True, help="dir with all relevant result folder")
    parser.add_argument('--label_file', type=str, default="", help="pickle file containing dict/trie that maps "
                                                                   "label to its frequency")
    ...
    for curr_dir in tqdm(all_dirs):
        ...
        try:
            ...
            analyse(gt_filename=gt_file, pred_filename=pred_file, label_trie=label_trie, only_seen_labels=only_seen_labels)
            analyse(gt_filename=gt_file, pred_filename=pred_file, label_trie=defaultdict(int), only_seen_labels=False)
