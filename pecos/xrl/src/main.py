...
#     def __init__(self, tp=0, his=False, k=None, Na=50, Ns=200, gam=0.1):
#         etp = ('KL', 'rKL', 'alpha', 'tsa')
#         self.Na = Na
#         self.Ns = Ns
#         self.gam = gam
#         self.k = k if k else Ns // 3
#         self.P = np.zeros([Na, Ns, Ns])
#         self.alpha = None
#         self.result = None
...
#         pi = np.ones([self.Ns, self.Na]) / self.Na if not self.train_from_history else self.result
...
#             v_pi = np.linalg.solve(Idt - self.gam * P_pi, r_pi)
#             # print(np.mean(v_pi))
...
#                     tmp = self.P[a] @ v_pi
#                     for s in range(self.Ns):
#                         pi_new[s, a] = (pi[s, a] ** (1 - eta)) * np.exp(eta * (self.r[s, a] + self.gam * tmp[s]))
#
#             if self.etp == 'rKL':
#                 for a in range(self.Na):
#                     tmp = self.P[a] @ v_pi
...
#                         pi_new[s, a] -= eta * 1 * (self.r[s, a] - v_pi[s] + self.gam * tmp[s])
#                         # 1 seems to work better then Na
#                         pi_new[s, a] = 1 / pi_new[s, a]
#
#             if self.etp == 'alpha':
#                 for a in range(self.Na):
#                     tmp = self.P[a] @ v_pi
...
#                             * (self.r[s, a] - v_pi[s] + self.gam * tmp[s])
#                         pi_new[s, a] = pi_new[s, a] ** (2 / (self.alpha - 1))
#
#             if self.etp == 'tsa':
#                 for a in range(self.Na):
#                     tmp = self.P[a] @ v_pi
#                     for s in range(self.Ns):
#                         pi_new[s, a] = (1 - eta) * (pi[s, a] ** (self.tsa - 1))
#                         pi_new[s, a] -= (1 - self.tsa) / self.tsa * eta * (self.r[s, a] - v_pi[s] + self.gam * tmp[s])
#                         pi_new[s, a] = pi_new[s, a] ** (1 / (self.tsa - 1))
#
#             for s in range(self.Ns):
...
#         # for s in range(self.Ns):
#         #     for a in range(self.Na):
#         #         ss += d_pi[s] * pi[s, a] * self.r[s, a]
#         return np.dot(d_pi, r_pi)


class PolicyGradientNew:
    def __init__(self, tp=0, his=False, k=None, kernel_type='toy1', Na=50, Ns=200, gam=0.99, eps=10**-12):
        if kernel_type == 'toy3':
            Na = 2
        etp = ('KL', 'rKL', 'alpha')
    ...
    def train(self, eta=1, T=20):
        ...
        for itr in range(T):
            ...
            if self.etp == 'KL':
                pi_new = pi ** (1 - eta)
                for a in range(self.Na):
                    pi_new[:, a] = pi_new[:, a] * np.exp(eta*(self.r[:, a] - (Idt-self.gam*self.P[a]) @ v_pi)/self.tau)
    ...
    def make_fig_exact(self, label='KL', linewidth=0.5):
        ...
        ax.plot([0, len(err0)-1], [0, np.log(2)*(len(err0)-1)], label='slope=log2', color='g', linestyle='dashed', linewidth=0.5)
        ax.set_xlabel('iterations', fontsize=22)
    ...
    # def value_iteration_old(self):
    #     V_pre = -np.ones(self.Ns)
    #     V = np.zeros(self.Ns)
    #     Q = np.zeros([self.Ns, self.Na])
    #     cnt = 0
    #     while np.linalg.norm(V - V_pre) / np.linalg.norm(V) > 10 ** -15:
    #         cnt += 1
    #         V_pre = V
    #         for a in range(self.Na):
    #             Q[:, a] = self.r[:, a] + self.gam * (self.P[a, :, :] @ V)
    #         Z = softmax(Q/self.tau, axis=1)
    #         HZ = 0
    #         if self.etp == 'KL':
    #             HZ = np.sum(xlogy(Z, Z), axis=1)
    #         if self.etp == 'rKL':
    #             HZ = np.sum(-np.log(Z), axis=1) / self.Na
    #         if self.etp == 'alpha':
    #             HZ = 4 / (1 - self.alpha ** 2) - 4 / (1 - self.alpha ** 2) * self.Na ** ((self.alpha - 1) / 2) * \
    #                    np.sum(Z ** ((self.alpha + 1) / 2), axis=1)
    #         V = np.max(Q, 1) - self.tau * HZ
    #     for a in range(self.Na):
    ...
    # def train_primal_dual_part(self, CV=0.1, tol=10**-6, step=0.01):  # too slow
    #     V = np.zeros(self.Ns)
    #     W = np.ones(self.Ns)
    #     Z0 = np.zeros([self.Ns, self.Na])
    #     Z = np.ones([self.Ns, self.Na]) / self.Na
    ...
    #
    #         EV = V - (Idt - self.gam * PZ.T) @ W / CV
    #         EW = (RZ - (Idt - self.gam * PZ) @ V - self.tau * HZ)
    #         EZ = np.zeros([self.Ns, self.Na])
    #         for a in range(self.Na):
    #             EZ[:, a] = (self.r[:, a] - (Idt - self.gam * self.P[a, :, :]) @ V) / self.tau - np.log(Z[:, a])
    #         Z0 = Z
    ...
    def train_primal_dual(self, CV=0.01, tol=10**-6, step=0.01):
        CM = self.tau
        V = np.ones(self.Ns)
    ...
    def train_primal_dual_batch(self, CV=0.01, tol=1e-5, step=1e-4, nb1=1, nb2=1):  # primal dual method from samples
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
            ...
            for s in s_idx:
                for a in range(self.Na):
                    ...
                    for sample in samples:
                        tmp_v += V[sample]
                    tmp_v /= nb2
                    H[s, a] = (H[s, a] + dt * ((self.r[s, a] - V[s] + self.gam * tmp_v) / CM)) / (1 + dt)
            # H = (H + dt * EM) / (1 + dt)
    ...
    def train_primal_dual_batch_acc(self, CV=0.01, tol=1e-5, step=1e-4, nb1=1, nb2=1, lam=0.01, m=5):  # "acc" = anderson acceleration
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
        ...
        while np.linalg.norm(M-M0) / np.linalg.norm(M) > tol or np.linalg.norm(EV) > tol:
            ...
            if 1 < cnt:
                ...
                V_h.append(V)
                U = np.array(GV_h).T
                c = np.linalg.solve(U.T @ U + lam * np.identity(np.size(U, 1)), np.ones(np.size(U, 1)))
                c = c / np.sum(c)
                V = c @ V_h
                if cnt > m:
                    GV_h = GV_h[1:]
                ...
                for s in s_idx:
                    for a in range(self.Na):
                        ...
                        for sample in samples:
                            tmp_v += V[sample]
                        tmp_v /= nb2
                        gH[s, a] = (H[s, a] + dt * ((self.r[s, a] - V[s] + self.gam * tmp_v) / CM)) / (1 + dt) - H[s, a]
    ...
    def train_primal_dual_batch_acc_explicit(self, CV=0.01, tol=1e-5, step=1e-4, nb1=1, nb2=1, lam=0.01, m=5):  # explicit version
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
                ...
                for s in s_idx:
                    for a in range(self.Na):
                        ...
                        for sample in samples:
                            tmp_v += V[sample]
                        tmp_v /= nb2
                        gH[s, a] = ((1-dt) * H[s, a] + dt * ((self.r[s, a] - V[s] + self.gam * tmp_v) / CM)) - H[s, a]
                GH_h.append(gH.reshape(-1))
                H = H + gH
                H_h.append(H.reshape(-1))
    ...
    def train_primal_dual_new(self, CV=0.01, tol=10**-6, step=0.01):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
    ...
    def train_primal_dual_new_continue(self, CV=0.01, tol=10**-6, step=3e-4):
    ...
    def train_primal_dual_new_continue_acc(self, CV=0.01, tol=10**-6, step=3e-4, lam=0.001, m=5):
        CM = self.tau if self.tau >= 0.01 else 2 ** (int(np.log2(0.01/self.tau)) + 1) * self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
    ...
    def train_primal_dual_epart(self, tol=10**-6, step=0.01):  # gradient descent for v, natural gradient descent for mu
        CM = self.tau
        V = np.ones(self.Ns)
    ...
    def train_primal_dual_e(self, tol=10**-6, step=0.01):  # vanilla gradient descent
        CM = self.tau
        V = np.ones(self.Ns)
        vec_e = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
    ...
    # def train_primal_dual_pseudo(self, CV=0.01, tol=10**-6, step=0.01):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
    #     CM = self.tau
    #     V = np.ones(self.Ns)
    #     M = np.ones([self.Ns, self.Na]) / self.Na
    #     M0 = np.zeros([self.Ns, self.Na])
    ...
    #                 - dt * np.ones(self.Na) * np.dot(tmp_pi, EM[s, :] + tmp_w - H[s, :])
    #
    #         for s in range(self.Ns):
    #             for a in range(self.Na):
    #                 M[s, a] = np.exp(H[s, a])
    ...
    #             self.lyap.append(self._lyap(CV, V, M))
    #
    #     for s in range(self.Ns):
    #         Z[s, :] = M[s, :] / np.sum(M[s, :])
    ...
    # def train_primal_dual_pseudo_alt(self, CV=0.01, tol=10**-6, step=0.01):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
    #     CM = self.tau
    #     V = np.ones(self.Ns)
    #     M = np.ones([self.Ns, self.Na]) / self.Na
    #     M0 = np.zeros([self.Ns, self.Na])
    ...
    #
    #     while np.linalg.norm(M-M0) / np.linalg.norm(M) > tol:
    #         tmp_err = np.linalg.norm(M-M0) / np.linalg.norm(M)
    #         M0[:] = M[:]
    #         cnt += 1
    #
    #         if tmp_err < 1e-4:
    ...
    def train_primal_dual_pseudo_mix(self, CV=0.01, tol=10**-6, step=0.01, c=0.99):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
        CM = self.tau
        V = np.ones(self.Ns)
        V0 = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
    ...
    def train_primal_dual_pseudo_mix_random_grad(self, CV=0.01, tol=1e-4, step=0.01, c=0.99, sigma=0.01):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
        ...
        while tmp_err > tol:
            ...
            EV = np.zeros(self.Ns)
            for a in range(self.Na):
                EV += (Idt - self.gam * self.P[a].T) @ M[:, a] + np.random.normal(loc=0.0, scale=sigma, size=self.Ns)
            EV /= CV
            V = (1 - dt) * V + dt * EV

            EM = np.zeros([self.Ns, self.Na])
            for a in range(self.Na):
                EM[:, a] = (self.r[:, a] - (Idt - self.gam * self.P[a]) @ V) / CM + np.random.normal(loc=0.0, scale=sigma, size=self.Ns)
    ...
    def train_primal_dual_pseudo_mix_sample(self, CV=0.01, tol=10**-4, step=0.01, c=0.99, n_sam=100, n_batch=1000):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
        CM = self.tau
        V = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
    ...
    def train_primal_dual_pseudo_mix_continue(self, CV=0.1, tol=10**-6, step=0.01, c=0.99):  # with \mu\log\hat{\mu} regularization and a different approximate Hessian
        # CM = self.tau
        CM_start = 0.01
        CM = self.tau if self.tau >= CM_start else 2 ** (int(np.log2(CM_start / self.tau)) + 1) * self.tau
        V = np.ones(self.Ns)
        V0 = np.ones(self.Ns)
        M = np.ones([self.Ns, self.Na]) / self.Na
        M0 = np.zeros([self.Ns, self.Na])
    ...
    # def train_hybrid_acc(self, CV=0.01, tol=10**-6, lam=0.01, m=5, step=1e-3):
    #     CM = self.tau
    #     V = np.ones(self.Ns)
    #     M = np.ones([self.Ns, self.Na]) / self.Na
    #     M0 = np.zeros([self.Ns, self.Na])
    ...
    #     # def f(V_i, H_i):
    #     #     Idt = np.identity(self.Ns)
    #     #     M_i = np.zeros([self.Ns, self.Na])
    #     #     for s_i in range(self.Ns):
    #     #         for a_i in range(self.Na):
    #     #             M_i[s_i, a_i] = H_i[s_i, a_i] if H_i[s_i, a_i] >= 1 else np.exp(H_i[s_i, a_i] - 1)
    #     #
    #     #     V_r = np.zeros(self.Ns)
    #     #     for a_i in range(self.Na):
    #     #         V_r += (Idt - self.gam * self.P[a_i].T) @ M_i[:, a_i]
    #     #     V_r /= CV
    #     #
    #     #     H_r = np.zeros([self.Ns, self.Na])
    #     #     for a_i in range(self.Na):
    #     #         H_r[:, a_i] = (self.r[:, a_i] - (Idt - self.gam * self.P[a_i]) @ V_i) / CM
    #     #
    #     #     return V_r, H_r, V_r-V_i, H_r-H_i
    #
    #     V_h = []
    #     GV_h = []
    #     GH_h = []
    ...
    #
    #         if cnt == 1:
    #             V, H, GV, GH = self._f_step(CV, CM, V, H, step)
    #             GV_h.append(GV)
    #             V_h.append(V)
    #             GH_h.append(GH.reshape(-1))
    #             H_h.append(H.reshape(-1))
    ...
    #
    #         err = np.linalg.norm(M - M0, 'fro') / np.linalg.norm(M, 'fro')
    #         tmp_err = np.max([err, np.linalg.norm(GV), np.linalg.norm(GH)])
    #         self.err_pd.append(err)
    #         if self.resultZ is not None:
    #             for s in range(self.Ns):
    #                 # if np.sum(M[s, :]) == 0:
    ...
    # def train_primal_dual_norm(self, CV=0.01, tol=10**-8, step=0.01):
    #     CM = self.tau
    #     V = np.ones(self.Ns)
    #     M = np.ones([self.Ns, self.Na]) / self.Na
    #     M0 = np.zeros([self.Ns, self.Na])
...
p.train_primal_dual_pseudo_mix_sample(CV=0.1, step=1e-3, tol=1e-3, c=0.9, n_sam=10000, n_batch=100000)
